# 💰 BillPro - GST Billing System with Authentication
...
#### 2. Setup Files
1. Copy all project files to `C:\xampp\htdocs\billpro\` (Windows)
   or `/opt/lampp/htdocs/billpro/` (Linux)
...
└── salesbilling_with_auth.sql  # Database with users table
```
...
5. Click "Choose File" and select **`salesbilling_with_auth.sql`** (NEW FILE with users table)
6. Click "Go" to import the database

**IMPORTANT**: Use the new SQL file `salesbilling_with_auth.sql` which includes the users table!
...
- Check URL: `http://localhost/billpro/`
...
5. Verify you're using the correct SQL file (salesbilling_with_auth.sql)
...
### Updated Files
- `salesbilling_with_auth.sql` - NEW database with users table
